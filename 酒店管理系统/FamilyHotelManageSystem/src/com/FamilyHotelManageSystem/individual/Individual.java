 ...
 *##############################################################################
 */
package com.FamilyHotelManageSystem.individual;
...
import com.FamilyHotelManageSystem.mainframe.HotelFrame;
import com.FamilyHotelManageSystem.mainframe.RightTopPanel;
...
import com.FamilyHotelManageSystem.sunsdk.swing.TJMoneyField;
import com.FamilyHotelManageSystem.sunsdk.swing.TJTextField;
import com.FamilyHotelManageSystem.sunsdk.swing.sunswing;


public class Individual extends JDialog implements ActionListener, MouseListener {
	...
	private void addRoom() {
		...
		if(arows.length > 0) {
			...
			"再追加", "友情提示", JOptionPane.INFORMATION_MESSAGE);
			return;
		}//endif
	}
	...
	private void subRoom() {
		...
		if(arows.length > 0) {
			for (int i = 0; i < arows.length; i++) {
				...
				if(!sqlCode[ar].equals(lbA.getText())) {		
					//判断主房间不能删除
					...
					" ] 房间是主房间，不能移除 ...", "友情提示", JOptionPane.INFORMATION_MESSAGE);
					zRooms = ar;
					return;
				}//Endif
			}//Endfor
			...
			"再移除", "友情提示", JOptionPane.INFORMATION_MESSAGE);
			return;
		}//endif
	}
	...
	private void saveLiveIn() {
		...
		try {
		    ...
		    if(livins < tb2.getRowCount() * 2) {
		    	...
		    	"连接或联系管理员", "友情提示", JOptionPane.INFORMATION_MESSAGE);
		    	//如果事务失败的话，则恢复状态图片
		...
		try {
			...
			if(o == tf1) {					//证件编码框获得焦点时回车
				...
				if(isValidity()) {
					...
					lbA.getText() + " ] 为主房间，开设房间吗？", "友情提示", JOptionPane.YES_NO_OPTION);
				...
				if(zRooms < 4) {					//检查房间数量
					...
					JOptionPane.showMessageDialog(null, "最多只能追加四间客房，" +
					"入住五间以上客房请使用团体开单", "友情提示", JOptionPane.INFORMATION_MESSAGE);
					return;
				}//Endif
			}else if(o == bt4) {
				if(zRooms >= 0) {					//检查房间数量

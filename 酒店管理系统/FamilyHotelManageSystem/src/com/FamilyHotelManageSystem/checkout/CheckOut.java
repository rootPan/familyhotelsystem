 ...
 *##############################################################################
 */
package com.FamilyHotelManageSystem.checkout;
...
public class CheckOut extends JDialog implements ActionListener, MouseListener {
	...
	public CheckOut(JFrame frame) {
		...
		this.setResizable(false);		//改变窗口大小(NO),不许改变
		pack();
		sunswing.setWindowCenter(this);	//设置窗口屏幕居中
	}
	...
	private void saveCKO() {
		long ckPK = sunsql.getPrimaryKey();			//获得结算记录
		String chNO = suntools.getNumber(suntools.Number_JS);	//获得结算单号
		String chkTime = Journal.getNowDTime();		//计算结算时间
		String reMark  = tf3.getText();				//备注信息

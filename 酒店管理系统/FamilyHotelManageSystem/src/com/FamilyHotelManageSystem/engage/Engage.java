...
public class Engage extends JDialog implements ActionListener,MouseListener {
	...
	private boolean initMrt() {
		...
		if(row == -1) {
			JOptionPane.showMessageDialog(null, "请在宾客预定信息表中指定记录！","友情提示", JOptionPane.INFORMATION_MESSAGE);
			return false;
		}
	...
	private boolean delInfo (int dr[]) {
		...
		if(rowCount > 0) {					//判断选择记录数
			int isDel = JOptionPane.showConfirmDialog (null, "确定要删除预订记录吗?", "友情提示", JOptionPane.YES_NO_OPTION);

 ...
 *##############################################################################
 */
package com.FamilyHotelManageSystem.mainframe;
...
import com.FamilyHotelManageSystem.about.About;
import com.FamilyHotelManageSystem.checkout.CheckOut;
import com.FamilyHotelManageSystem.customer.Customer;
import com.FamilyHotelManageSystem.engage.Engage;
import com.FamilyHotelManageSystem.individual.Individual;
import com.FamilyHotelManageSystem.menu.ApartBill;
import com.FamilyHotelManageSystem.menu.Change;
import com.FamilyHotelManageSystem.menu.GoOn;
import com.FamilyHotelManageSystem.menu.Record;
import com.FamilyHotelManageSystem.menu.UniteBill;
import com.FamilyHotelManageSystem.query.Query;
import com.FamilyHotelManageSystem.setup.NetSetup;
import com.FamilyHotelManageSystem.setup.Setup;
...
import com.FamilyHotelManageSystem.team.Team;



public class HotelFrame extends JFrame implements ActionListener, MouseListener, Runnable {
	...
	private void buildBott () {
		...
		lb1 = new JLabel("    Family 酒 店 管 理 系 统    ");
		lb2 = new JLabel("    当前操作员 :  " + userid + "                  ");
		lbA = new JLabel(clue + "请选择功能项 ...   　　　　　　　　　　　　　　　　　　　　");
		lbB	= new JLabel(face + "Family 酒 店 管 理 系 统 -    内部测试 版         ");
		...
		try {
			...
			if(!rs.next()) {		//如果无结果集，提示用户刷新房间数据
				if(LeftTopPanel.title1.getText().length() == 0) {
					...
					" ] 房间信息已更改，请刷新房间信息，再为宾客开设房间", "友情提示", JOptionPane.INFORMATION_MESSAGE);
				}
				return false;
			}else {
	...
	public void actionPerformed (ActionEvent ae) {
		...
		lbB.setText(face + "Family酒 店 管 理 系 统 -    内部测试版         ");
	}
	...
	private static final String URL="jdbc:mysql://localhost:3306/familyhotelsystem";
	private static final String USER="root";
	private static final String PASSWORD="wyj690920";
	public void run() {
		try {
			Statement ste1 = null;
			...
			if(sunini.getIniKey ("Default_Link").equals ("1")) {		//JDBC连接方式
				...
				user1="root";
				pwd1="wyj690920";
				dbf1="familyhotelsystem";
				ip1="localhost";
				String url = "jdbc:mysql//localhost:3306/familyhotelsystem";


//				String url1  = "jdbc:sqlserver://" + ip1 + ":" + acc1 + ";" + "databasename=" + dbf1;
				//注册驱动
				DriverManager.registerDriver (new com.FamilyHotelManageSystem.microsoft.jdbc.sqlserver.SQLServerDriver());
				//获得一个连接
